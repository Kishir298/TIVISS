...
def test_identity_is_stable_across_model_changes():
    identity = AgentIdentity.create(agent_id="tiviss-x", name="tiviss", version="0.1.0", owner_id="kishir")
...
def test_identity_validation_rejects_unknown_state():
    with pytest.raises(IdentityValidationError):
        AgentIdentity.create(name="tiviss", version="0.1.0", owner_id="kishir", state="flying")
...
def test_identity_validation_accepts_aware_timestamp():
    ...
    assert identity.created_at == datetime(2026, 1, 1, tzinfo=timezone.utc)
...
def test_identity_validation_of_existing_record():
    identity = AgentIdentity.create(agent_id="tiviss-1", name="tiviss", version="0.1.0", owner_id="k")
